...
If there's no prompt yet, ask for it. Call `agent_delegate` once per CLI in the pool with `permission: "read-only"` (always — Compare never writes), in parallel, and answer the same prompt yourself directly if Claude is in the pool. **Show every worker's full answer, each under its own heading — do not summarize or paraphrase any worker's answer instead of showing it, including your own.** Only after every full answer is shown, add a short agreement/disagreement callout underneath — that callout is in addition to the full answers, never a replacement for showing one. If a worker failed (empty output, timeout, an auth/credit error, etc.), say so plainly under its heading instead of silently dropping it from the comparison.
